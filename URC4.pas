///////////////////////////////////////////////////////////////////////////////
// Nombre: RC4
// Objetivo: Implementa el algoritmo criptográfico RC4.
...
interface
uses
  Classes, Sysutils, UCryptBase;

type
  TRC4= class(TDCP_cipher)
  protected
...
class function TRC4.GetID: integer;
begin
end;

class function TRC4.GetAlgorithm: string;
begin
Result:= 'RC4';
end;

class function TRC4.GetMaxKeySize: integer;
begin
Result:= 2048;
end;

class function TRC4.SelfTest: boolean;
const Key1: array[0..4] of byte= ($61,$8A,$63,$D2,$FB);
      InData1: array[0..4] of byte= ($DC,$EE,$4C,$F9,$2C);
      OutData1: array[0..4] of byte= ($F1,$38,$29,$C9,$DE);
var Cipher: TRC4;
    Data: array[0..4] of byte;
begin
Cipher:= TRC4.Create(nil);
...
procedure TRC4.Init(const Key; Size: longword; InitVector: pointer);
var i, j, t: longword;
...
var i, j, t, k: longword;
begin
if not fInitialized then
   raise EDCP_cipher.Create('No se ha inicializado el cifrador.');
i:= 0; j:= 0;
...
begin
if not fInitialized then
   raise EDCP_cipher.Create('No se ha inicializado el cifrador.');   //'No se ha inicializado el cifrador.'
i:= 0; j:= 0;
for k:= 0 to Size-1 do
    begin
    i:= (i + 1) and $FF;
